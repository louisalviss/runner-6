...
if len(sys.argv) != 5:
    raise SystemExit('usage: render-dsh-anime-aura-v6-zerocrop.py <video> <music> <output.mp4> <analysis.json>')
...
# Align strongest early sound rise to aura reveal.
...
for idx,(start,end,speed,role,target_d) in enumerate(segments):
    ...
    filters.append(f'[{base}a]scale=1080:1920:force_original_aspect_ratio=increase:flags=lanczos,crop=1080:1920,gblur=sigma=38:steps=2,eq=brightness=-0.34:saturation=0.78[{bg}]')
    if role=='aura_farm':
        # The only intentional crop: known centered Sukuna close-up, used as the aura payoff.
        filters.append(f'[{base}b]scale=-2:1920:flags=lanczos,crop=1080:1920:x=(iw-1080)/2:y=0,eq=contrast=1.08:saturation=1.12,vignette=PI/5[hero{idx}]')
        filters.append(f'[{bg}][hero{idx}]overlay=0:0:shortest=1,setsar=1,fps=30[{out}]')
    else:
        # ZERO-CROP fight/stutter/payoff: exact source aspect ratio, full width, nothing lost.
        bright=0.045 if role.startswith('stutter_') and idx%2==0 else 0.0
        filters.append(f'[{base}b]scale=1080:-2:flags=lanczos,eq=contrast=1.07:saturation=1.10:brightness={bright}[full{idx}]')
        # Slight vertical jitter only on stutter; source frame itself is untouched.
        yoff='(H-h)/2'
        if role.startswith('stutter_'):
            yoff=f'(H-h)/2+{6 if idx%2==0 else -6}'
        filters.append(f'[{bg}][full{idx}]overlay=x=0:y={yoff}:shortest=1,setsar=1,fps=30[{out}]')
...
flash_expr='+'.join([f'between(t,{x:.3f},{x+0.040:.3f})' for x in cut_times])
filters.append(f"[vcat]drawbox=x=0:y=0:w=iw:h=ih:color=white@0.18:t=fill:enable='{flash_expr}',fade=t=in:st=0:d=0.06,fade=t=out:st={out_duration-.22:.3f}:d=0.22[vout]")
...
if source_has_audio:
    filters.append('[sfxcat]volume=0.18,highpass=f=100[sfx]'); filters.append('[music][sfx]amix=inputs=2:duration=first:dropout_transition=0,alimiter=limit=0.96[aout]')
...
meta={'status':'success','concept':'zero_crop_fast_fight_stutter_aura_payoff','duration_sec':round(out_duration,3),'aura_reveal_sec':round(aura_reveal,3),'detected_music_drop_sec':round(drop_sec,3),'audio_start_sec':round(audio_start,3),'drop_alignment_error_sec':round(abs((drop_sec-audio_start)-aura_reveal),3),'fight_layout':'exact source aspect ratio scaled to 1080px width; zero crop','aura_layout':'intentional centered hero crop only','format':'1080x1920 H.264 + AAC','bytes':output.stat().st_size}
